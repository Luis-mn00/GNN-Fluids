...
def plot_3D(z_net, z_net_init, z_gt, save_dir, var=5):
    T = z_net.size(0)
    ...
    ax1.set_title('Ground Truth (z_gt)'), ax1.grid()
    ax1.set_xlabel('X'), ax1.set_ylabel('Y'), ax1.set_zlabel('Z')
    ax2.set_title('Thermodynamics-informed GNN (z_net)'), ax2.grid()
    ax2.set_xlabel('X'), ax2.set_ylabel('Y'), ax2.set_zlabel('Z')
    ax3.set_title('Thermodynamics-informed GNN with starting model (z_net_init)'), ax3.grid()
    ...
    q1_init, q2_init, q3_init = z_net_init[0, :, 0], z_net_init[0, :, 2], z_net_init[0, :, 1]

    var_gt, var_net, var_init = z_gt[-1, :, var], z_net[-1, :, var], z_net_init[-1, :, var]
    var_error = var_gt - var_net
    var_error_min, var_error_max = var_error.min(), var_error.max()
    ...
    # Scatter points
    s1 = ax1.scatter(q1_gt, q2_gt, q3_gt, c=var_gt, s=100, alpha=0.5, vmax=z_max, vmin=z_min)
    s2 = ax2.scatter(q1_net, q2_net, q3_net, c=var_net, s=100, alpha=0.5, vmax=z_max, vmin=z_min)
    s3 = ax3.scatter(q1_init, q2_init, q3_init, c=var_init, s=100, alpha=0.5, vmax=z_max, vmin=z_min)
    ...
    def animate(snap):
        ...
        ax2.cla()
        ax3.cla()
        ax1.set_title(f'Ground Truth (z_gt), frame={snap}'), ax1.grid()
        ax1.set_xlabel('X'), ax1.set_ylabel('Y'), ax1.set_zlabel('Z')
        ax2.set_title(f'Thermodynamics-informed GNN (z_net), frame={snap}'), ax2.grid()
        ax2.set_xlabel('X'), ax2.set_ylabel('Y'), ax2.set_zlabel('Z')
        ax3.set_title(f'Thermodynamics-informed GNN with starting model (z_net_init), frame={snap}'), ax3.grid()
        ax3.set_xlabel('X'), ax3.set_ylabel('Y'), ax3.set_zlabel('Z')
        ...
        # Scatter points for the current frame
        q1_gt, q2_gt, q3_gt = z_gt[snap, :, 0], z_gt[snap, :, 2], z_gt[snap, :, 1]
        q1_net, q2_net, q3_net = z_net[snap, :, 0], z_net[snap, :, 2], z_net[snap, :, 1]
        q1_init, q2_init, q3_init = z_net_init[snap, :, 0], z_net_init[snap, :, 2], z_net_init[snap, :, 1]

        var_gt, var_net, var_init = z_gt[snap, :, var], z_net[snap, :, var], z_net_init[snap, :, var]

        ax1.scatter(q1_gt, q2_gt, q3_gt, c=var_gt, s=100, alpha=0.5, vmax=z_max, vmin=z_min)
        ax2.scatter(q1_net, q2_net, q3_net, c=var_net, s=100, alpha=0.5, vmax=z_max, vmin=z_min)
        ax3.scatter(q1_init, q2_init, q3_init, c=var_init, s=100, alpha=0.5, vmax=z_max, vmin=z_min)
...
def video_plot_3D(z_net, z_net_init, z_gt, save_dir, n=[]):
    ...
    generate_pointclud(z_net_init, n, name='net_init')  # Add z_net_init visualization
    ...
    for i in range(z_net.shape[0] - 1):
        try:
            ...
            frame_net_init = cv2.cvtColor(cv2.imread(f'images/net_init_frame_{i + 1}.png'), cv2.COLOR_BGR2RGB)  # Add z_net_init frame
            ...
            frame_net_init = cv2.resize(frame_net_init[100:-100, 450:-450, :], None, fx=0.8, fy=0.8)  # Resize z_net_init frame

            # Ensure all images have the same height
            height = min(frame_gt.shape[0], frame_net.shape[0], frame_net_init.shape[0])

            # Concatenate images horizontally
            concatenated_image = np.concatenate((frame_gt[:height, :], frame_net[:height, :], frame_net_init[:height, :]), axis=1)
            image_lst.append(concatenated_image)
        except:
            print(f"Error processing frame {i + 1}")

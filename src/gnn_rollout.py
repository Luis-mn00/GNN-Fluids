...
class NodalGNN(pl.LightningModule):
    ...
    def decoder(self, x, edge_attr, src, dest):
        ...
        Medges = torch.bmm(M, torch.transpose(M, 1, 2)) / (torch.max(M) + 1e-8)  # forzamos que la M sea SDP
    ...
    def pass_thought_net(self, z_t0, z_t1, edge_index, n, f, batch=None, mode='val', plot_info = []):
        ...
        #dxdt, dvdt, dEdt = dzdt[:, 0:3], dzdt[:, 3:6], dzdt[:, 6]
        #dxdt_net, dvdt_net, dEdt_net = dzdt_net[:, 0:3], dzdt_net[:, 3:6], dzdt_net[:, 6]
        
        #loss_x = self.criterion(dxdt, dxdt_net)
        #loss_v = self.criterion(dvdt, dvdt_net)
        #loss_E = self.criterion(dEdt, dEdt_net)
        
        #loss_z = loss_x + 0.1*loss_v + loss_E

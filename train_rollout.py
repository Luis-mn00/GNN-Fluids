...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain_weights', default=r'epoch=34-val_loss=49.53.ckpt', type=str, help='name')
    ...
    parser.add_argument('--output_dir_exp', default=r'outputs_rollout/', type=str, help='output directory')
    ...
    val_set = GraphDataset(dInfo, os.path.join(args.dset_dir, 'dataset_5', dInfo['dataset']['datasetPaths']['val']))
    val_dataloader = DataLoader(val_set, batch_size=dInfo['model']['batch_size'])
    test_set = GraphDataset(dInfo, os.path.join(args.dset_dir, 'dataset_5', dInfo['dataset']['datasetPaths']['test']), length=60)
    ...
    save_folder = f'outputs_rollout/runs/{name}'
    ...
    checkpoint_best = ModelCheckpoint(dirpath=save_folder, filename='{epoch}-{val_loss:.2f}', monitor='val_loss',
                                      save_top_k=3)  # Save best models
    checkpoint_last = ModelCheckpoint(dirpath=save_folder, filename='last_epoch', monitor='epoch',
                                      save_top_k=1, mode='max')  # Save last epoch model
    ...
    trainer = pl.Trainer(#accelerator="cpu",
                         accelerator="gpu",
                         logger=wandb_logger,
                         callbacks=[checkpoint_best, checkpoint_last, lr_monitor, rollout, early_stop],
                         profiler="simple",
